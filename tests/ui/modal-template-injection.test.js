  ...
  runner.addTest(
    ...
    async () => {
      if (typeof ModalTemplateManager === "undefined") {
        return { skip: true, details: "Skipped: ModalTemplateManager not available" };
      }
      try {
        const mtm = new ModalTemplateManager();
        const modal = document.createElement("div");
        modal.className = "modal";
        modal.innerHTML = '<div class="modal-content"><div class="modal-header"><h2 data-modal-title></h2></div><div class="modal-body" data-modal-content></div></div>';
        ...
        const cNested = cBody ? cBody.querySelector(".modal-content") !== null : false;
        ...
        const tNested = tBody ? tBody.querySelector(".modal-content") !== null : false;
        ...
        const lNested = lBody ? lBody.querySelector(".modal-content") !== null : false;
        const lValid = lBody && !lNested && lBody.innerHTML.trim() !== "";

        mtm.cleanup();

        const ok = cOk && tOk && lOk && cValid && tValid && lValid && /Conway|Termite|Langton/.test(cTitle + tTitle + lTitle);
        return { passed: ok, details: ok ? "injection ok" : "injection failed" };
      } catch (e) {
