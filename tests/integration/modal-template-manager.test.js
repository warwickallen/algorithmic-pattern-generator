  ...
  runner.addTest(
    ...
    async () => {
      if (typeof ModalTemplateManager === "undefined") {
        return { passed: true, details: "Skipped: ModalTemplateManager not available" };
      }
      try {
        const modalTemplateManager = new ModalTemplateManager();
        const available = modalTemplateManager.getAvailableSimulations();
        const hasAll = available.includes("conway") && available.includes("termite") && available.includes("langton");
        ...
        const ok = hasAll && hasConway && hasTermite && hasLangton && !hasInvalid && !!conwayContent && !!termiteContent && !!langtonContent && !invalidContent;
        modalTemplateManager.cleanup();
        return { passed: ok, details: ok ? "templates ok" : "templates missing" };
      } catch (e) {
